...
export default class extends DiscordEvent {
    ...
    async exec(channel: DMChannel | GuildChannel) {
        ...
        if (this.client.managers.blacklist.getServer(channel.guild.id) ||
        ...
        const guild = await this.client.managers.guilds.fetch(channel.guild.id, channel.guild.name);
        ...
        if (channel.guild.me.hasPermission('VIEW_AUDIT_LOG')) {
            ...
            const log = logs.entries.find(l => l.target instanceof GuildChannel && l.target.id === channel.id);
