...
export default class extends DiscordEvent {
    ...
    async exec(dguild: Guild, user: User) {
        ...
        if (!guild.logging_enabled && !guild.logging_member_ban_remove) {
            return;
        }
        ...
        const embed = new MessageEmbed()
            ...
            .setDescription(`<@${user.id}> / ${user.tag} / ${user.id} has been unbanned`)
        ...
        if (dguild.me.hasPermission('VIEW_AUDIT_LOG')) {
            ...
            const logs = await dguild.fetchAuditLogs({ limit: 1, type: 'MEMBER_BAN_REMOVE' });
