...
export default class extends DiscordEvent {
    ...
    async exec(message: Message) {
        ...
        if (this.client.managers.blacklist.getServer(message.guild.id) ||
        ...
        const embed = new MessageEmbed()
            ...
            .addField('Content', `\`\`\`\n${Escapes.backticks(Utils.shorten(message.cleanContent, 1000))}\n\`\`\``);
        ...
        if (message.guild.me.hasPermission('VIEW_AUDIT_LOG')) {
            ...
            const log = logs.entries.find(l => l.target instanceof Message && l.target.id === message.id);

            if (log && log.executor.id !== this.client.user.id) {
                embed.setFooter(`Deleted by ${log.executor.tag} / ${log.executor.id}`, log.executor.displayAvatarURL({ dynamic: true }));
            }
