...
export default class extends DiscordEvent {
    ...
    public async exec(oldRole: Role, newRole: Role) {
        if (
            this.client.managers.blacklist.getServer(newRole.guild.id) ||
        ...
        if (!guild.logging_enabled || !guild.logging_role_update) return;
        ...
        if (oldRole.name !== newRole.name) {
            embed.addField(
                ...
                `Before: \`${Escapes.backticks(
                    oldRole.name,
                )}\`\nAfter: \`${Escapes.backticks(newRole.name)}\``,
                true,
            );
        }
        ...
        if (newRole.guild.me?.hasPermission('VIEW_AUDIT_LOG')) {
            ...
            const log = logs.entries.find(
                (l) => l.target instanceof Role && l.target.id === newRole.id,
            );
