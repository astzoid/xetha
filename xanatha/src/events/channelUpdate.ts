...
import { DMChannel, GuildChannel, MessageEmbed, TextChannel, VoiceChannel } from 'discord.js';
...
export default class extends DiscordEvent {
    ...
    async exec(oldChannel: DMChannel | GuildChannel, newChannel: DMChannel | GuildChannel) {

        if (oldChannel.type === 'dm' || newChannel.type === 'dm') {
            return;
        }

        if (this.client.managers.blacklist.getServer(newChannel.guild.id) ||
            this.client.managers.blacklist.getUser(newChannel.guild.ownerID)) {
            return;
        }

        const guild = await this.client.managers.guilds.fetch(newChannel.guild.id, newChannel.guild.name);
        ...
        const embed = new MessageEmbed()
            ...
            .setDescription(`<#${newChannel.id}> was updated\nType: \`${newChannel.type}\``);
        ...
        if (newChannel.type === 'text' && oldChannel.type === 'text') {
            ...
            if (oC.nsfw !== nC.nsfw) {
                embed.addField('NSFW', nC.nsfw ? `<:yes:800415381340684330>` : `<:no:800415449488556053>`);
            }

            if (oC.topic !== nC.topic) {
                embed.addField('Topic', `Before: \`${Escapes.backticks(oC.topic)}\`\nAfter: \`${Escapes.backticks(nC.topic)}\``);
            }
        }
        ...
        if (newChannel.type === 'voice' && oldChannel.type === 'voice') {
            ...
            if (oC.bitrate !== nC.bitrate) {
                embed.addField('Bitrate', `Before: \`${oC.bitrate}\`\nAfter: \`${nC.bitrate}\``);
            }
        }
        ...
        const uniques = oldOverwrites.length > newOverwrites.length ? differentiate(oldOverwrites, newOverwrites) : differentiate(newOverwrites, oldOverwrites);
        ...
        if (newOverwrites.length > oldOverwrites.length) {
            ...
            ((newChannel instanceof TextChannel && oldChannel instanceof TextChannel) &&
                (newChannel.topic !== oldChannel.topic || newChannel.nsfw !== oldChannel.nsfw))) {
            type = 11;
        } else {
            type = 14;
        }
        ...
        for (const overwrite of newOverwrites) {
            ...
            const oldOverwrite = oldOverwrites.find(o => o.id === overwrite.id);
            ...
            const permJsonOld = oldOverwrite.toJSON() as { [k: string]: boolean; };
            ...
            if (newChannel.permissionOverwrites.map(o => `${o.allow}|${o.deny}`).toString() === oldChannel.permissionOverwrites.map(o => `${o.allow}|${o.deny}`).toString()) {
                continue;
            }
            ...
            if (overwrite.type === 'member') {
                ...
                const role = newChannel.guild.roles.cache.find(r => r.id === overwrite.id);
            ...
            for (const perm of differentPerms) {
                if (permJsonNew.hasOwnProperty(perm) && permJsonOld.hasOwnProperty(perm)) {

                    if (permJsonNew[perm] === true && permJsonOld[perm] === false) {
                        field.value += `\n${`<:yes:800415381340684330>`} ${perm}`;
                    } else if (permJsonNew[perm] === false && permJsonOld[perm] === true) {
                        field.value += `\n${`<:no:800415449488556053>`} ${perm}`;
                    }

                } else if (permJsonNew.hasOwnProperty(perm) && !permJsonOld.hasOwnProperty(perm)) {
        ...
        if (newChannel.guild.me.hasPermission('VIEW_AUDIT_LOG')) {
            ...
            const logs = await newChannel.guild.fetchAuditLogs({ limit: 5, type });
            const log = logs.entries.find(l => l.target instanceof GuildChannel && l.target.id === newChannel.id);
