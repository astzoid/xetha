...
export default class extends Command {
    constructor(client: Disclosure) {
        super(client, {
            name: 'npm',
            description: 'Sends or Shows the information of a Public NPM (Node Package Manager) Package',
            cooldown: 10,
            args: 1,
            usage: ['npm <package>'],
            aliases: [],
    ...
    async execute(message: Message, argv: Arguments) {
        ...
        try {
            ...
            const maintainers = Utils.trimArray(body.maintainers.map((user: any) => user.name));
            const dependencies = version.dependencies ? Utils.trimArray(Object.keys(version.dependencies)) : null;
