...
export default function MemberRoute(manager: DisclosureSharder) {
    ...
    route.get('/:guild_id/:member_id', AsyncWrapper(async (req, res) => {

        const guild_id = req.params.guild_id;
        const member_id = req.params.member_id;

        const blacklisted = await manager.broadcastEval(`this.managers.blacklist.getUser('${member_id}')`, 0) as BlacklistedAttributes;
        ...
        if (!data) data = new Member({ guild_id, member_id, tag: member.user.tag });
    ...
    route.post('/:guild_id/:member_id', AsyncWrapper(async (req, res) => {
        ...
        if (!data) {
            return res.status(404).json({ message: 'Guild Member Data Not Found' });
        }
