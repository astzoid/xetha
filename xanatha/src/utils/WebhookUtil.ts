...
import { Guild, MessageEmbed, TextChannel, Webhook, Collection } from 'discord.js';
...
export default class WebhookUtil {
    ...
    static async send(client: Disclosure, guild: Guild, channel_id: string, embed: MessageEmbed | string) {
        ...
        if (channel && channel.type === 'text') {
            ...
            if (WebhookCache.has(channel.id)) {
            ...
            } else if (channel.permissionsFor(guild.me).has('MANAGE_WEBHOOKS')) {
                ...
                if (webhooks.size) {
                    ...
                    await channel.createWebhook('Xetha', { reason: 'Logging' })
