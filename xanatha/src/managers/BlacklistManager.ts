...
import { BlacklistedServer, BlacklistedUser, BlacklistedAttributes } from '../database/models/Blacklisted';
import { Collection, User } from 'discord.js';
import { Disclosure } from 'disclosure-discord';

export default class BlacklistManager extends Manager<Collection<string, BlacklistedAttributes>> {
    ...
    private broadcast(type: 'server' | 'user', target_id: string) {
        this.client.shard.broadcastEval(`this.emit('blacklist', ['${type}', '${target_id}'])`);
    }
