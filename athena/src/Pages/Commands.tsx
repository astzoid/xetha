...
export default function Commands() {
  ...
  const emptyRows =
    rowsPerPage - Math.min(rowsPerPage, commands.length - page * rowsPerPage);
  ...
  const results = useMemo(() => {
    ...
    return commands.filter(
      (command) =>
        ...
        command.aliases.some((alias) => alias.match(pattern)),
    );
  }, [search]);
  ...
  return (
    <Content>
      <Meta
        title="Commands"
        description="List of all the bot's Commands"
        url="/commands"
        keywords={['commands', 'command']}
      />
      {status === 200 ? (
        <Container>
            ...
            {search.length ? (
              <>
                <Typography variant="body1" color="textSecondary">
                  Found {results.length} result{results.length > 1 ? 's' : null}{' '}
                  for "{search}"{' '}
                </Typography>
                {results.length === 1 ? (
                  <Meta
                    ...
                    url={`/commands?search=${encodeURIComponent(
                      results[0].name,
                    )}`}
                    keywords={[
                      'command',
