...
export class RequestHandlerClass {
    ...
    public request<T>(method: RequestMethod, options: RequestOptions): Promise<APIResponse<T>> {
    ...
    private async execute(request: APIRequest): Promise<APIResponse> {
        ...
        if (res.ok) {
            return new APIResponse(res, request.method, request.options, await parseResponse(res));
        }
...
const RequestHandler = new RequestHandlerClass('http://127.0.0.1:3002', process.env.SECRET_KEY);
