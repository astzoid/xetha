...
export default class Manager {
    ...
    public static async guild(guild_id: string): Promise<Guild | Blacklisted> {
        ...
        const response = await REST.request<Guild>('GET', { route: `/api/guild/${guild_id}` })
            .then((res) => res)
            .catch((err: APIError) => err);

        if (response instanceof APIResponse) {

            response.body.settings = Serialize(response.body.settings, ['guild_id', 'name']);
    ...
    public static async member(guild_id: string, member_id: string): Promise<Member | Blacklisted> {

        const response = await REST.request<Member>('GET', { route: `/api/member/${guild_id}/${member_id}` })
            .then((res) => res)
            .catch((err: APIError) => err);

        if (response instanceof APIResponse) {

            response.body.data = Serialize(response.body.data, ['guild_id', 'member_id', 'tag']);
    ...
    public static updateMember(guild_id: string, member_id: string, data: any) {
        return REST.request('POST', { route: `/api/member/${guild_id}/${member_id}`, data });
    }

    public static async user(user_id: string): Promise<User | Blacklisted> {

        const response = await REST.request<User>('GET', { route: `/api/user/${user_id}` })
            .then((res) => res)
            .catch((err: APIError) => err);
