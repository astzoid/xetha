...
export default function Sanitize(
  guild: Guild,
  raw: Partial<GuildSettings & UnionSettings>,
) {
  ...
  if (
    typeof raw !== 'object' ||
    Array.isArray(raw) ||
    !Keys(Object.keys(guild.settings), raw)
  )
  ...
  for (const key in raw) {
    ...
    if (stringArrayProperties.includes(key)) {
      const clone = (typeof raw[key] === 'string'
        ? [raw[key]]
        : raw[key]) as Array<string>;
        ...
        for (const threshold of clone) {
          if (typeof threshold === 'object' && !Array.isArray(threshold)) {
            if (['mute', 'kick', 'softban', 'ban'].includes(threshold.action)) {
              threshold.action = String.prototype.toLowerCase.call(
                threshold.action,
              );
